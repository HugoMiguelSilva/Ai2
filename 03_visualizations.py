...
ax.hist([low_risk_income, high_risk_income], bins=50, label=['Low Risk', 'High Risk'],
...
ax.scatter(low_risk['loan_amnt'], low_risk['person_income'], 
          alpha=0.6, s=40, label='Low Risk', color='#2ecc71', edgecolors='none')
ax.scatter(high_risk['loan_amnt'], high_risk['person_income'], 
          alpha=0.6, s=40, label='High Risk', color='#e74c3c', edgecolors='none')
ax.set_xlabel('Loan Amount ($)')
ax.set_ylabel('Person Income ($)')
ax.set_title('Loan Amount vs Income by Risk Category')
ax.legend()
ax.grid(True, alpha=0.25)
plt.tight_layout()
...
ax.set_ylabel('Frequency')
ax.set_title('Interest Rate Distribution by Risk Category')
ax.legend()
ax.grid(True, alpha=0.25)
plt.tight_layout()
